...
if(${USE_QT} EQUAL 6)
    # Find Qt6
    find_package(Qt6 ${MINIMUM_QT6_VERSION} COMPONENTS Core Widgets REQUIRED)
    set(QT_LRELEASE_EXECUTABLE "${_qt6Core_install_prefix}/bin/lrelease")
...
function(kumir2_use_qt)
    ...
    if(${USE_QT} EQUAL 6)        
        find_package(Qt6 ${MINIMUM_QT6_VERSION} COMPONENTS ${_QT_COMPONENTS} REQUIRED)
        foreach(component ${_QT_COMPONENTS})
            include_directories(${Qt6${component}_INCLUDE_DIRS})
            list(APPEND _QT_LIBRARIES ${Qt6${component}_LIBRARIES})
            list(APPEND _QT_INCLUDES ${Qt6${component}_INCLUDE_DIRS})
        endforeach(component)
    elseif(${USE_QT} EQUAL 5)        
...
function(kumir2_qt_moc INFILE FILE_LOCATION)
	...
	if(APPLE AND ${USE_QT} EQUAL 6)
		file(APPEND ${PARAMS_FILE_NAME} "-F${Qt6_DIR}/../../\n")
    elseif(APPLE AND ${USE_QT} EQUAL 5)
...
function(kumir2_add_launcher)    
    ...
    if(WIN32 AND EXISTS "${ICONS_DIR}/${PARSED_ARGS_APP_ICON_NAME}.ico")
        ...
        if(${USE_QT} EQUAL 6)
            list(APPEND QT_LIBRARIES ${Qt6Core_QTMAIN_LIBRARIES})
        elseif(${USE_QT} EQUAL 5)
...
function(kumir2_add_tool)
    ...
    if(WIN32 AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${PARSED_ARGS_NAME}.rc")
        ...
        if(${USE_QT} EQUAL 6)
            list(APPEND PARSED_ARGS_LIBRARIES ${Qt6Core_QTMAIN_LIBRARIES})
        elseif(${USE_QT} EQUAL 5)
